...
export class SpiritSeedGenerator {
  ...
  static async generateFromSkeletons(skeletons: SpiritSeedSkeleton[]): Promise<Array<Omit<Material, 'id'>>> {
    ...
    return skeletons.map((skeleton, index) => {
      ...
      const plant: SpiritFieldPlantSnapshot = { id: globalThis.crypto?.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(36).slice(2)}`, ...identity, quality: skeleton.rank, minRealm: balance.minRealm, stageDurationMs: balance.stageDurationMs, baseYieldMin: balance.baseYield[0], baseYieldMax: balance.baseYield[1] };
