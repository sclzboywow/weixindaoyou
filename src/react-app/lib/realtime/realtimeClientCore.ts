...
export type RealtimeConnectionState =
  'idle' | 'connecting' | 'online' | 'reconnecting' | 'offline' | 'blocked';
...
// Browser behavior remains the default; other clients adapt only the transport.
...
export type RealtimeTransport = {
  open(channels: RealtimeChannel[]): RealtimeSocket;
  isOnline(): boolean;
};
...
export class RealtimeClient {
  ...
  private intentionallyClosedSockets = new WeakSet<RealtimeSocket>();
  private listeners = new Map<
    RealtimeServerEventType,
    Set<(event: RealtimeServerEvent) => void>
  >();
  ...
  constructor(
    ...
    private readonly resolveUrl?: (channels: RealtimeChannel[]) => string,
  ) {
  ...
  enableChannel(channel: RealtimeChannel) {
    ...
    this.status.channels[channel] = {
      ...
      enabled: true,
      state:
        this.status.connection === 'idle' ? 'idle' : this.status.connection,
    };
    this.emitStatus();
    if (!this.identityKey) {
      return;
  ...
  connect() {
    ...
    if (
      ...
      this.updateStatus('offline', {
        lastError: this.transport ? '网络离线' : '浏览器离线',
      });
      return;
    }
    ...
    try {
      socket = this.transport
        ? this.transport.open(channels)
        : (new WebSocket(
            this.resolveUrl!(channels),
  ...
  private updateStatus(
    ...
    this.status = {
      ...
      channels: {
        ...
        'player-state': this.nextChannelStatus(
          'player-state',
          connection,
          patch,
        ),
